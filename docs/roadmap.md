...
> **本轮结论（2026-08-14）**：P3-1、P3-2、P3-2A 与 P3-5 已完成；P3-3 以
> “通过·已接受风险”收尾，严格干净 VPS 与真实生产业务验收仍未满足；P3-4 经确认取消。
> 因此本轮 P3 按已确认范围完成，但不等同于下面所有原始严格验收项均已通过。

### P3-1 恢复计划 `internal/restore/plan.go` ✅ 已完成
...
### P3-2 恢复执行 `internal/restore/execute.go` ✅ 已完成
...
### P3-2A 隔离恢复命名与端口映射 ✅ 已完成

为非空目标机与自动演练提供显式 `--isolate` 模式：稳定派生 Compose project、container、
volume、network、files root 和 Docker 自动端口，并通过 isolation label 与 `state.json` 完成
可校验、可重试的精确 cleanup。普通 restore 仍在冲突时 fail closed，不会自动切换隔离模式。

**验收**：生产项目与隔离副本可并存；中断后同一 isolation ID 能续跑；cleanup 只删除已证明
归属的隔离资源，生产容器、端口、volume、network 和文件基线保持不变。

---

### P3-3 跨机重建实测（一次性任务，但最重要） ⚠️ 通过·已接受风险

开一台干净的 VPS，加进清单，从 hub 完整走一遍恢复，把踩到的每个坑写回 `design.md`。

本轮实际在一台已有业务容器的 destination 上使用隔离模式完成跨主机恢复。source independence、
受限验证栈的数据、应用读写、加密字段、镜像 digest、幂等续跑与清理均通过；但 destination
不是干净 VPS，也没有恢复真实生产业务栈。用户已明确接受 `CHK-001` 与 `FBK-001`，风险接受
不表示严格验收已通过或问题已修复。

**必须验证的点**：

- [x] 受限验证栈的数据完整（抽查表记录和最新数据）
- [x] 受限验证栈的应用能正常读写
- [x] 受限验证栈的加密字段能正常解密
- [x] 恢复出来的镜像 digest 和备份时一致
- **未满足（已接受风险）**：新机器上只安装 Docker、Compose v2 与 sshd，没有 Ark、restic
- [x] 全程只用了「hub + 对象存储凭证 + restic 密码」，
      没有依赖原机器上的任何东西
...
实现结果：Hub 每分钟复用健康投影，告警状态持久化到 schema v3，首次/复发立即发送、
持续故障 24 小时静默、恢复通知一次；`ark backup` 通过独立成功/失败端点发送外部心跳，
其投递结果不改变备份事实或退出码。真实钉钉、外部监控宽限期和 systemd 停启仍需上线环境验收。
