...
从离线介质取回 SSH 私钥 → 恢复完毕。P3-4 原计划实测这条路径，但因缺少独立干净机器且
现有宿主机没有隔离 VM 空间，本轮经确认取消；未来验证时必须提供独立故障域并重新立项。
...
目标机只要有 Docker、Compose v2 和 sshd，并且已经加入当前清单的 `hosts`。P3 MVP 不接受命令行临时
SSH 参数，避免恢复入口绕过清单校验、主机密钥策略和目标定义一致性检查。
...
- **告警**：超时未备份、连续失败、演练失败。对接钉钉机器人并持久化 24 小时静默期，
  首次发生和复发立即通知，恢复时通知一次。
...
CLI 初始化或重置，不提供未鉴权的 Web 安装入口。密码以 Argon2id 哈希保存在独立的
`/var/lib/ark-hub/auth.json`，该文件不写入 `ark.db`、不进入 hub 自备份；会话只保存在
...
产物目录只提交一个占位文件，构建结果不进版本库；没有真实产物时页面回落到
「请用 `make hub` 构建」的提示页，而不是让整个 Hub 起不来——前端缺失不影响
API、登录和备份调度，把服务拦在门外是过度反应。

内嵌控制台之后，`ark-hub` 的 CSP 从「禁止一切脚本」放宽为 `script-src 'self'`。
这是引入 SPA 的必然代价，也是这一阶段唯一实质扩大的攻击面，因此策略整串由测试
精确断言：收紧会让控制台白屏，放宽会悄悄扩大脚本来源，两个方向都不能靠 review 漏掉。
`style-src` 保留 `'unsafe-inline'` 是明确取舍——Vue 的 `:style` 绑定需要它，
而静态嵌入的 HTML 无法为每次请求注入 nonce。

**入口 HTML 与静态资源同样要求登录**，未认证者拿不到控制台 JS，也就拿不到 API 形状。
history 模式路由所需的 SPA fallback 必须发生在鉴权之后：未知的非 API 路径返回入口 HTML，
`/api/` 的未知路径仍然返回 404 JSON。登录与退出保持服务端表单路径，不为前端新增
JSON 登录端点——鉴权契约在 P4-1 已严格验收，没有理由为了界面重开一个入口。

总览的大小趋势来自主机摘要新增的 `last_backup_bytes` 与 `recent_backup_sizes`
（最多 14 点）。健康投影本来就取了运行记录，加字段近乎零成本；采样只统计成功与
warn 的 run，因为失败 run 的字节数是半截数据，混进趋势会掩盖 ADR-011 要盯的体积腰斩。

**死人开关**：`ark-hub` 是常驻进程，它自己哑火了不会有人知道。
备份调度不依赖它（ADR-005），但告警依赖它。因此 `ark backup` 每次跑完
向外部监控推一次心跳——由外部系统发现「ark 整个没动静了」。心跳使用供应商无关的
成功/失败双端点：`ok`、`warn` 调成功端点，`fail` 和清单加载后的前置失败调失败端点。
发送发生在锁、状态库和 manifest 全部收尾之后，结果只进入摘要的
`heartbeat_status=disabled|sent|failed`，不会改写 run、manifest 或退出码。

钉钉与心跳秘密统一放在 `monitoring.env_file` 指向的 root-only 文件中。文件必须是当前用户
所有的普通文件、权限不超过 `0600` 且不能是符号链接；URL 默认只允许 HTTPS。Hub 每分钟
复用同一健康投影评估告警，投递成功后才刷新静默时间。通知成功但状态写入前进程退出时，
重启后可能重复一次；这里明确选择 at-least-once，避免把真实故障永久漏掉。
