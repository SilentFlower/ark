...
- [ ] 示例 hub 条目覆盖清单和一致性状态库，同时明确排除密码与私钥
- [ ] 并发写入 WAL 数据库时导出的副本通过 integrity_check 且包含已提交数据
- [ ] 导出失败、取消和清理错误均可见，不残留宽权限临时文件
- [ ] doctor 在无法自动核对对象锁时稳定输出 warn 而非 ok/fail
- [ ] 运维文档包含离线密钥、对象锁、空间回收和 hub 重建材料
- [ ] make check、构建、无 CGO 构建和 git diff 检查通过
